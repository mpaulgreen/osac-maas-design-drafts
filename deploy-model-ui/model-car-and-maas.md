...
metadata:
  ...
  annotations:
    opendatahub.io/display-name: "NVIDIA A10G (24GB)"
    opendatahub.io/description: "Single NVIDIA A10G GPU with 24GB VRAM — suitable for 7-8B parameter models"
...
Patch the OdhDashboardConfig to enable MaaS and deployment tools. See [dashboard-config.md](dashboard-config.md) for the full flag reference.
...
| Flag | Purpose |
|------|---------|
| `modelAsService` | MaaS UI — subscriptions, auth policies, model publishing |
| `maasAuthPolicies` | Authorization policy management from dashboard |
| `vLLMDeploymentOnMaaS` | Enables "Publish as MaaS" checkbox in the deploy wizard and vLLM runtime option |
| `observabilityDashboard` | MaaS observability — token consumption, rate-limit violations |
| `deploymentWizardYAMLViewer` | Form/YAML toggle in deploy wizard for real-time YAML preview |
| `llmGatewayField` | Gateway selector in deploy wizard — choose which gateway routes the model's traffic |

> **No DSC patch needed.** All 5 flags above only require `kserve` and `kserve.modelsAsService` to be `Managed` in the DSC — already done in Phase 3a of the [MaaS deployment guide](../maas/README.md). The `llamastackoperator` DSC patch is only needed if you enable `genAiStudio` (for Playground/AI asset endpoints).
...
After patching, refresh the OpenShift AI dashboard. The left navigation should now show **Subscriptions** and **Authorization policies** under Settings.
...
3. Fill in:
   - **Name**: `granite-free-subscription`
...
2. Fill in:
   - **Name**: `granite-premium-subscription`
   - **Model**: Select `granite-3-1-8b-fp8` from `granite-serving`
...
1. Click **Create subscription** again
2. Fill in:
   - **Name**: `granite-enterprise-subscription`
   - **Model**: Select `granite-3-1-8b-fp8` from `granite-serving`
...
> **Note**: The OpenShift AI dashboard does not support API key creation for external OIDC users ([RHOAI 3.4 docs, Section 1.13.1.3](../maas/Red_Hat_OpenShift_AI_Self-Managed-3.4-Govern_LLM_access_with_Models-as-a-Service-en-US.pdf)). External OIDC (Keycloak) users must create API keys via curl or other HTTP clients. The dashboard's **Gen AI studio → API keys** page can view and revoke existing keys, but not create new ones for external OIDC users. This is a Technology Preview limitation.
...
# Expected: Key starts with sk-oai-, Subscription: granite-premium-subscription
...
```bash
curl -sk "${HOST}/granite-serving/granite-3-1-8b-fp8/v1/chat/completions" -H "Authorization: Bearer ${API_KEY}" -H "Content-Type: application/json" -d '{"model":"granite-3-1-8b-fp8","messages":[{"role":"user","content":"What is Kubernetes in one sentence?"}],"max_tokens":50}' | python3 -c "import sys,json; d=json.load(sys.stdin); print('Model:', d.get('model')); print('Response:', d['choices'][0]['message']['content']); print('Tokens:', d.get('usage'))"
...
```bash
curl -sk "${HOST}/granite-serving/granite-3-1-8b-fp8/v1/completions" -H "Authorization: Bearer ${API_KEY}" -H "Content-Type: application/json" -d '{"model":"granite-3-1-8b-fp8","prompt":"OpenShift is","max_tokens":50}' | python3 -c "import sys,json; d=json.load(sys.stdin); print('Response:', d['choices'][0]['text'][:200]); print('Tokens:', d.get('usage'))"
